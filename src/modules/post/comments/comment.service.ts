...
class CommentService {
  ...
  async replies(commentId: string, query: PaginationDTO, viewerId?: string) {
    ...
    return this.listFor(
      { post: parent.post, parentComment: parent._id },
      query,
      viewerId,
    );
  }
  ...
  async delete(commentId: string, userId: string) {
    ...
    await Promise.all([
      CommentLikeModel.deleteMany({ comment: { $in: ids } }),
  ...
  private async listFor(
      ...
      CommentModel.aggregate([
        ...
        { $unwind: "$author" },
        {
          $lookup: {
            from: "commentlikes",
            let: { id: "$_id" },
            pipeline: [{ $match: { $expr: { $eq: ["$comment", "$$id"] } } }],
            as: "likes",
          },
        },
        {
    ...
    return {
      ...
      pagination: {
        ...query,
        total: totals,
        totalPages: Math.ceil(totals / query.limit),
      },
    };
  }
}
