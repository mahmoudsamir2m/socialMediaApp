...
router.post(
  ...
  validation(postIdSchema),
  async (req: Request, res: Response) => {
    const data = await postLikeService.like(
      req.params.id as string,
      req.user!.id,
    );
...
router.delete(
  ...
  validation(postIdSchema),
  async (req: Request, res: Response) => {
    const data = await postLikeService.unlike(
      req.params.id as string,
      req.user!.id,
    );
...
router.post(
  "/:id/share",
  authMiddleware,
  validation({ ...postIdSchema, ...createPostSchema }),
  async (req: Request, res: Response) => {
    const data = await postShareService.share(
      req.params.id as string,
      req.user!.id,
      req.body.content,
    );
...
router.post(
  "/:postId/comments",
  authMiddleware,
  validation({ ...postCommentIdSchema, ...commentSchema }),
  async (req: Request, res: Response) => {
