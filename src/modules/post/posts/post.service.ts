...
import { BadRequestException, ForbiddenException, NotFoundException } from "../../../common/exceptions/applications.exceptions";
...
const asArray = (value?: string | string[]) => {
  ...
  try { const parsed: unknown = JSON.parse(value); return Array.isArray(parsed) ? parsed.filter((id): id is string => typeof id === "string") : [value]; }
  catch { return [value]; }
};

class PostService {
  private async upload(files: Express.Multer.File[], userId: string) {
    const uploaded = await imageService.uploadImages({ userId: `posts/${userId}`, files });
  ...
  async create(author: string, data: CreatePostDTO, files: Express.Multer.File[]) {
    if (!data.content && files.length === 0) throw new BadRequestException("Post must include content or at least one image");
    const images = files.length ? await this.upload(files, author) : [];
    try { return await PostModel.create({ author, ...(data.content ? { content: data.content } : {}), images }); }
    catch (error) { await imageService.deleteImages(images.map((image) => image.publicId)); throw error; }
  }

  async getById(id: string, viewerId?: string) {
    if (!await PostModel.exists({ _id: id })) throw new NotFoundException("Post not found");
  ...
  async getFeed(query: PaginationDTO, viewerId?: string) {
    ...
    const [posts, total] = await Promise.all([
      PostModel.find().sort({ createdAt: -1, _id: -1 }).skip(skip).limit(query.limit).select("_id").lean(),
  ...
  async update(id: string, author: string, data: UpdatePostDTO, files: Express.Multer.File[]) {
    const post = await PostModel.findById(id);
    if (!post) throw new NotFoundException("Post not found");
    if (post.author.toString() !== author) throw new ForbiddenException("You cannot update this post");
    ...
    try {
      ...
      if (!post.content && post.images.length === 0) throw new BadRequestException("Post must include content or at least one image");
      await post.save();
    } catch (error) { await imageService.deleteImages(uploaded.map((image) => image.publicId)); throw error; }
  ...
  async delete(id: string, author: string) {
    const post = await PostModel.findById(id);
    if (!post) throw new NotFoundException("Post not found");
    if (post.author.toString() !== author) throw new ForbiddenException("You cannot delete this post");
    const sharedPosts = await PostModel.find({ originalPost: id }).select("_id").lean();
    const postIds = [post._id, ...sharedPosts.map((shared) => shared._id)];
    const comments = await CommentModel.find({ post: { $in: postIds } }).select("_id").lean();
    const commentIds = comments.map((comment) => comment._id);
    await Promise.all([
      PostLikeModel.deleteMany({ post: { $in: postIds } }),
      CommentModel.deleteMany({ post: { $in: postIds } }),
      ...(commentIds.length ? [CommentLikeModel.deleteMany({ comment: { $in: commentIds } })] : []),
