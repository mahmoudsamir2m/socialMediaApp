...
jest.mock("../src/database/model/post.model", () => ({
  PostModel: { exists: jest.fn() },
}));
jest.mock("../src/database/model/comment.model", () => ({
  CommentModel: { exists: jest.fn() },
}));
jest.mock("../src/database/model/story.model", () => ({
  StoryModel: { exists: jest.fn() },
}));
jest.mock("../src/database/model/message.model", () => ({
  MessageModel: { exists: jest.fn() },
}));
